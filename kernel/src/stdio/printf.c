	...
	for(; *format != '\0'; format++, char_cnt++) {
		...
		if(*format == '%') {
			...
			case 'u':
				...
				break;
			case 'f':
			case 'F': /*no float provided yet*/
				break;
			case 'e':
			case 'E': /*no float provided yet*/
				break;
			case 'a':
			case 'A': /*no float provided yet*/
				break;
			case 'g':
			case 'G': /*no float provided yet*/
				break;
			case 'n': /*see above*/
				break;
...
int32_t printi(INT_TYPES t, va_list ap) {
	...
	switch(t) {
	/*currently no testing if input is correct. rn hu and hd
	                 * are the same*/
	case S_CHAR:
	case U_CHAR:
